...
public class ArbolTareas {
	...
	public int coste(String idTarea){
		BinaryTreeNode zeregina = find(root, idTarea);
		return coste(zeregina);
	}
	...
	private int coste(BinaryTreeNode a){
		if(a==null) {
			...
			return a.element.coste;
		}else {
			return a.element.coste + coste(a.left) + coste(a.right);
		}
	}
	
	
	private BinaryTreeNode find(BinaryTreeNode a, String idTarea) {
		if(a==null) {
			return null;
		}else if(a.element.id.equals(idTarea)) {
			return a;
		}else {
			BinaryTreeNode ezk = find(a.left,idTarea);
			if(ezk!=null) {
				return ezk;
			}else {
				return find(a.right,idTarea);
			}
		}
	}
